...
6. Cloudinary is the confirmed attachment provider. Attachments remain private references (`storageKey`, metadata); persist the provider asset ID, public ID, resource type, authenticated delivery type and version when implementing its adapter. Check the requesting user's access to the parent record before issuing a short-lived private download or proxy response. Implement upload authorization, content/size validation, and abandoned-upload cleanup as E4.
