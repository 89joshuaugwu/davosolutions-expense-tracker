...
export function AppShell({ children, role, name, sections, preview = false, section, month = "2026-09" }: Props) {
  ...
  return <div className="app-shell">
    ...
    {mobileOpen && <button className="sidebar-backdrop" aria-label="Close navigation" onClick={closeNavigation} />}
    <aside id="workspace-navigation" className={`sidebar ${mobileOpen ? "is-open" : ""}`} aria-label="Main navigation">
      <Link href={href("dashboard")} className="brand-link" aria-label="Davo Expenses home"><Brand /></Link>
      <button ref={closeButtonRef} className="icon-button mobile-close" onClick={closeNavigation} aria-label="Close navigation"><X size={20} /></button>
    ...
    <div className="app-main"><header className="topbar"><div className="breadcrumbs"><button ref={menuButtonRef} className="icon-button mobile-menu" onClick={() => setMobileOpen(true)} aria-label="Open navigation" aria-expanded={mobileOpen} aria-controls="workspace-navigation"><Menu size={21} /></button><span>Workspace</span><span className="breadcrumb-divider">/</span><strong>{navigation.find((item) => item.key === active)?.label ?? "Overview"}</strong></div><div className="topbar-right"><span className="private-label"><span className="status-dot" />{preview ? "Design preview" : "Internal workspace"}</span><span className="topbar-divider" /><span className="avatar small">{name.split(" ").map((part) => part[0]).slice(0, 2).join("")}</span></div></header>
      <main id="main-content" className="main-content">{children}</main><footer className="app-footer"><span>© {new Date().getFullYear()} Davo Solutions</span><span>Expenses & Profit Tracker <span className="footer-dot">·</span> Internal finance workspace</span></footer>
    </div>
  </div>;
}
